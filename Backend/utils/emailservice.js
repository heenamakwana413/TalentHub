...
const sendEmail = async({to, subject, htmlContent}) => {
    try{
        ...
        const response = await fetch("https://api.brevo.com/v3/smtp/email" , {
            ...
            body: JSON.stringify({
                sender: {name: "Talenthub", email: SENDER_EMAIL },
                to,
                subject,
                htmlContent,
            }),
        ...
        if(!response.ok) throw new Error(result.message || "Brevo API Errpr");
        return result
    }
    catch(error) {
...
export const sendVerificationEmail = async(email, name, otp) => {
    return sendEmail({
        to: [{email, name}],
        subject: "Your Verification Code - TalentHub",
